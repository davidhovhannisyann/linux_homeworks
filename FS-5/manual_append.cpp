...
int main(int argc, char** argv){
    ...
    int written1 = write(file_desc, str1.c_str(), str1.size()); // write to 1st line to 1st descriptor
    if(written1 < 0){
        cerr << strerror(errno) << endl;
        exit(errno);
    }

    int written2 = write(new_fd, str2.c_str(), str2.size()); // write to 2nd line to 2nd descriptor
    if(written2 < 0){
        cerr << strerror(errno) << endl;
        exit(errno);
    }
